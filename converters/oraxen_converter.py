...
class OraxenForItemsConverter(BaseConverter):
    def __init__(self) -> None:
        super().__init__()
        self.DIRECTORIES = self.config['directories']['oraxen']['items']
        self.INPUT_DIR = f'{self.INPUT_DIR}oraxen/items/'
        self.RESULT_DIR = f'{self.RESULT_DIR}oraxen/items/'
    ...
    def items_to_buy_pages(self, file: dict):
        for item_id, item_data in file.items():
                ...
                "items": {
                    "Yes": {
                        ...
                        "right_click_commands": [
                            f"[console] o give %player_name% {item_id} 1",
            ...
            for ing, count in self.recipes[item_id].items():
                ...
                if ing.islower():
                    requirement['material'] = self.ORAXEN_ITEM_MATERIALS[ing]['id']
                    requirement['modeldata'] = self.ORAXEN_ITEM_MATERIALS[ing]['model_data']
                    oraxen_material = self.ORAXEN_ITEM_MATERIALS[ing]['id'].lower()
                    model_data = self.ORAXEN_ITEM_MATERIALS[ing]['model_data']
                    result['items']['Yes']['left_click_commands'].append(
                        f'[console] minecraft:clear %player_name% minecraft:{oraxen_material}{{CustomModelData:{model_data}}} {count}'
                    )
    ...
    def convert(self):
        super().convert()
        print('All oraxen items for items were converted succesfully')


class OraxenForMoneyConverter(BaseConverter):
    def __init__(self) -> None:
        super().__init__()
        self.DIRECTORIES = self.config['directories']['oraxen']['money']
        self.INPUT_DIR = f'{self.INPUT_DIR}oraxen/money/'
        self.RESULT_DIR = f'{self.RESULT_DIR}oraxen/money/'
    ...
    def convert(self):
        super().convert()
        print('All oraxen items for money were converted to dmenu succesfully')
